...
:---:|---
 ...
 7 | Delete `enriched:good` [F] and `shredded:good` [K]. Rerun the *EmrEtlRunner* with either `--skip staging` option or with `--resume-from enrich`.
 ...
 10 | Rerun the *EmrEtlRunner* with `--skip staging,enrich,shred,elasticsearch` option.<br><br>**WARNING**: In R90+ if you pass `--skip shred` to EmrEtlRunner then RDB Loader does not load unstructured events and contexts. This issue is to be resolved in R92.
 ...
 12 | Rerun the *EmrEtlRunner* with `--resume-from archive_enriched` option.
