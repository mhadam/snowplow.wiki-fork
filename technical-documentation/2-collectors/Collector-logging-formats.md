...
Thus, the output of Kinesis-S3 is a projection of raw event data (serialized Thrift records, not enriched) in the form of a compressed LZO file. Each `.lzo` file has a corresponding `.lzo.index` file containing the byte offsets for the LZO blocks, so that the blocks can be processed in parallel using Spark.

**Note**: We also provide an option to GZIP the Thrift records rather than produce LZO files. However, those files are not meant to be processed in Spark. They are archives of Thrift records without the protocol buffers layer being applied.
